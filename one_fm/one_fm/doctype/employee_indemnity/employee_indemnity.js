...
var set_basic_salary = function(frm) {
	frappe.call({
		method: 'one_fm.one_fm.doctype.employee_indemnity.employee_indemnity.get_salary_for_employee',
		args: {'employee': frm.doc.employee},
		callback: function(r) {
			if(r && r.message){
				var basic_salary = r.message;
				if(basic_salary){
					frm.set_value('basic_salary', basic_salary);
				}
			}
			else{
				frappe.throw("Please set the Basic Salary in Salary Structure.");
			}
		}
	});	
}
	...
	if(frm.doc.employee && frm.doc.date_of_joining && frm.doc.exit_date && frm.doc.exit_status){
		frappe.call({
			...
			callback: function(r) {
				if(r && r.message){
					var data = r.message;
					var per_day_amount = frm.doc.basic_salary/26;
