...
def execute(filters=None):
    ...
    if ledger_data:
        unique_items = list(set([i.item_code for i in ledger_data])) #Remove Duplicates
        unique_names = frappe.get_all("Item",{"name":['in',unique_items]},['item_name','name']) #Fetch Item Names
        for each in unique_names:
            ...
            if stock_balance_dict.get(each.name):
                ...
                if not filters.get('show_only_issues'):
                    data.append({
                        'item_code':each.name,
