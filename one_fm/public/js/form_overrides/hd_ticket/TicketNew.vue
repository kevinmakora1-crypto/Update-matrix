...
const template = createResource({
  ...
  onSuccess: (data) => {
    ...
    const customFields = [
      {
        ...
        options: "Yes\nNo",
        default: "No",
        required: 1, // Changed from reqd to required for frontend validation
      },
      {
    ...
    // Prepend custom fields, avoiding duplicates if they somehow exist in customFields
    data.fields = [...customFields, ...data.fields];
...
function handleOnFieldChange(e: any, fieldname: string, fieldtype: string) {
  ...
  // Custom Logic
  if (fieldname === 'custom_reference_doctype') {
     console.log('Fetching filtered processes for:', e.value);
     call('one_fm.overrides.hd_ticket.get_filtered_processes', {
        doctype_name: e.value,
        is_doctype_related: 'Yes'
     }).then(res => {
        applyFilters('custom_process', res); 
        applyFilters('process', res);
        
        // Auto-select logic - apply AFTER filters to avoid reset
        // Filter application in formCustomisation.ts resets value to ""
        if (res && res.length === 1) {
             console.log('Auto-selecting process:', res[0]);
             // Check if 'process' exists in templateFields
            if (templateFields.hasOwnProperty('process')) {
                templateFields['process'] = res[0];
            } else if (templateFields.hasOwnProperty('custom_process')) {
                templateFields['custom_process'] = res[0];
            }
        }
     });
  } else if (fieldname === 'custom_is_doctype_related') {
    if (e.value === 'No') {
        templateFields['custom_reference_doctype'] = ""; // Reset reference doctype
        call('one_fm.overrides.hd_ticket.get_filtered_processes', {
            is_doctype_related: 'No'
        }).then(res => {
             applyFilters('process', null);
             applyFilters('custom_process', null);
        });
    }
  }
...
onMounted(async () => {
  await call("one_fm.overrides.hd_ticket.get_priority").then(res => priorities.value = res);
