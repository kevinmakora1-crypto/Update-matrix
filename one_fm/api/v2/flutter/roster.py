...
def assign_staff(employees, shift, request_employee_assignment=None):
    ...
    if len(validation_logs) > 0:
        frappe.log_error(message=str(validation_logs), title='Schedule Staff Validation Error')
        ...
        try:
            ...
            frappe.log_error(message=str(e), title='Error while assigning staff')
...
def get_roster_view(start_date: str, end_date: str, assigned: int = 0, scheduled: int = 0, employee_search_id: str = None, 
    ...
    try:
        ...
        frappe.log_error(message=frappe.get_traceback(), title='Get Roster View')
...
def edit_post(posts, values):
    ...
    try:
        ...
        frappe.log_error(message=frappe.get_traceback(), title='Edit Post')
