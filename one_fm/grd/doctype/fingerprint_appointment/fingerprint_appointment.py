...
class FingerprintAppointment(Document):
    ...
    def notify_grd_operator_of_rejection(self):
        ...
        subject = f'Fingerprint Appointment Rejected - {self.civil_id}'
        message = f"""
            <p>The fingerprint appointment request has been rejected.</p>
            
            <p><strong>Employee:</strong> {self.full_name} ({self.employee})<br>
            <strong>Civil ID:</strong> {self.civil_id}<br>
            <strong>Appointment Type:</strong> {self.fingerprint_appointment_type}<br>
            <strong>Rejection Reason:</strong> {self.reason_for_rejection}</p>
