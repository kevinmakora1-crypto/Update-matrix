...
def timesheet_automation(start_date=None,end_date=None,project=None):
    ...
    for key, group in itertools.groupby(logs, key=lambda x: (x['employee'])):
        ...
        for attendance in attendances:
            ...
            if not post:
                frappe.log_error(f"No Post Allocation found for {key} on {date} for shift {attendance.operations_shift}", "Timesheet Automation Error")
                continue
            ...
            start_time = frappe.get_list("Employee Checkin", {"employee": key, "time": ['between', (date, date)], "log_type": "IN"}, "time", order_by="time asc")
            if not start_time:
                frappe.log_error(message=f"No Check-in found for {key} on {date}", title="Timesheet Automation Error")
                continue
            start = start_time[0].time
            #Get end time from last employee checkin of that day of log type OUT
            end_time = frappe.get_list("Employee Checkin", {"employee": key, "time": ['between', (date, date)], "log_type": "OUT"}, "time", order_by="time desc")
            if not end_time:
                frappe.log_error(message=f"No Check-out found for {key} on {date}", title="Timesheet Automation Error")
                continue
            end = end_time[0].time
